...
class ENNMessage(nn.Module):
    ...
    def add_message(self, m, h, angles_idx=None):
        ...
        h_in = _gather_nodes(h, self.pairs_idx[:,:,1], self.n_h)
    ...
    def forward(self, x, bond_x, sc_x, eucl_dists, graph_dists, 
        ...
        for t, attn_layer in enumerate(self.attn_layers):
            x = self.mess_pass_layer(
                x, bond_x, sc_x, angles, mask, bond_idx, sc_idx, angles_idx, t)
...
class ContribsNet(nn.Module):
    ...
    def __init__(self, n_in, n_h, vec_in, act, dropout=0.0, 
                 layer_norm=True):
        super().__init__()
        contrib_head = create_contrib_head(n_in, n_h, act, dropout, layer_norm) 
...
class MyCustomHead(nn.Module):
    ...
    def __init__(self, n_input, n_h, n_h_contribs, pre_layers=[], 
        ...
        super().__init__()
        fc_pre = hidden_layer(n_input, n_h, False, dropout[0], norm, act)
        self.preproc = nn.Sequential(*fc_pre)
        fc_type = hidden_layer(n_h, n_input, False, dropout[1], norm, act)
        self.types_net = clones(nn.Sequential(*fc_type), self.N_TYPES)
        self.contribs_net = ContribsNet(
            n_input, n_h_contribs, n_h, act, dropout[2], layer_norm=norm)
    ...
    def forward(self, x, sc_types):
        ...
        for i in range(self.N_TYPES):
            ...
            else: x_types = x_types + 0.0 * self.types_net[i](x_) # fake call (only necessairy for distributed training - to make sure all proceesses have gradients for all parameters)
        x = x + x_types 
...
class Transformer(nn.Module):
    ...
    def forward(self, x, bond_x, sc_x, sc_m_x, eucl_dists, 
                ...
                angles_idx, sc_types):
        x = self.encoder(
            self.projection(x), bond_x, sc_x, eucl_dists, graph_dists, angles, 
